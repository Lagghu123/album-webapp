  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-8 h-full" onClick={() => setActiveMenuId(null)}>
      <section className="mb-10">
        <h2 className="text-2xl font-bold text-white mb-6">Flashback Memories</h2>
        <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
          <div className="relative w-48 h-64 flex-shrink-0 rounded-2xl overflow-hidden bg-surface-dark group cursor-pointer">
            <img alt="Flashback" className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity" src="https://images.unsplash.com/photo-1492691527719-9d1e07e534b4?w=400" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
            <div className="absolute bottom-4 left-4">
              <p className="text-white font-bold text-lg">3 Years Ago</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-sm text-slate-500">{albums.length} albums</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
          <div 
            onClick={() => setShowAlbumModal(true)}
            className="group flex flex-col items-center justify-center gap-4 rounded-2xl p-3 bg-surface-dark/30 border-2 border-dashed border-white/10 hover:border-primary/50 transition-all cursor-pointer min-h-[280px]"
          >
            ...
            <p className="text-white/50 font-medium group-hover:text-white">Create New Album</p>
          </div>
          ...
          {albums.map((album) => (
            <div 
              ...
              className="group flex flex-col gap-3 rounded-2xl p-3 bg-surface-dark border border-white/5 hover:border-primary/50 transition-all shadow-lg cursor-pointer"
            >
              <div className="relative aspect-video w-full rounded-xl overflow-hidden bg-slate-800">
                <img alt={album.title} className="w-full h-full object-cover" src={album.cover} />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                {album.private && (
                  <div className="absolute top-2 right-2 bg-black/50 p-1.5 rounded-lg">
              ...
              <div className="flex justify-between items-start px-1">
                <div>
                  ...
                  <p className="text-sm text-slate-400">{album.date}</p>
                </div>
                <button 
              ...
              {activeMenuId === album.id && (
                <div className="absolute right-4 bottom-12 z-50 w-40 bg-[#1e293b] border border-white/10 shadow-xl rounded-xl p-1" onClick={(e) => e.stopPropagation()}>
                  <button onClick={() => handleDeleteAlbum(album.id)} className="w-full text-left flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-red-400 hover:bg-red-500/10 transition-colors">
      ...
      <Modal isOpen={showAlbumModal} onClose={() => !isCreating && setShowAlbumModal(false)} title="Create New Album">
        <form onSubmit={handleCreateAlbum} className="flex flex-col gap-6">
          <div>
            <label className="text-xs font-semibold text-slate-300 uppercase block mb-2">Album Title</label>
            <input 
              type="text" 
              value={albumTitle}
          ...
          <div>
            <label className="text-xs font-semibold text-slate-300 uppercase block mb-2">Privacy</label>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
            <button type="button" onClick={() => setShowAlbumModal(false)} disabled={isCreating} className="px-4 py-2 text-sm text-slate-400">Cancel</button>
            <button type="submit" disabled={isCreating || !albumTitle.trim()} className="px-5 py-2 rounded-md text-sm font-bold bg-primary text-white hover:bg-blue-600 disabled:opacity-50">
